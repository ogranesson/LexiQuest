$(document).ready(function() {
    ...
    $(document).on('click', '#addConfirm', function(e) {
        if($('#selectMenu').val() != 0) {
            let $newCategory = '<div class="category flex row"><div class="text">' + $('#selectMenu option:selected').text() + '</div><button name="delete" class="deleteCategory" type="button" class="px-2">&times;</button></div>';
    ...
    $(document).on('click', '.deleteCategory', function(e) {
        ...
        let insertIndex = $select.toArray().findIndex(function(option) {
            return parseInt($(option).val(), 10) > $value; // findIndex returns the index of the first result that satisfies this condition
        });                                                // in this case, the value being higher than the value of the deleted option
